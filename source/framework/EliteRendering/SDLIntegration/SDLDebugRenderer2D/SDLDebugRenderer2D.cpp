...
void Elite::SDLDebugRenderer2D::DrawRect(const Elite::Rect rect, const Color& color, float depth)
{
	//Left bot to left top
	m_vLines.push_back(Vertex(rect.bottomLeft, depth, color));
	m_vLines.push_back(Vertex({ rect.bottomLeft.x, rect.bottomLeft.y + rect.height }, depth, color));

	//Left top to right top
	m_vLines.push_back(Vertex({ rect.bottomLeft.x, rect.bottomLeft.y + rect.height }, depth, color));
	m_vLines.push_back(Vertex({ rect.bottomLeft.x + rect.width, rect.bottomLeft.y + rect.height }, depth, color));

	//Right top to right bot
	m_vLines.push_back(Vertex({ rect.bottomLeft.x + rect.width, rect.bottomLeft.y + rect.height }, depth, color));
	m_vLines.push_back(Vertex({ rect.bottomLeft.x + rect.width, rect.bottomLeft.y }, depth, color));
