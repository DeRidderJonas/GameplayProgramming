...
namespace Elite
{
	...
	void InfluenceMap<T_GraphType>::PropagateInfluence(float deltaTime)
	{
		...
		else m_TimeSinceLastPropagation = 0.f;

		int amountOfNodes = GetNrOfNodes();
		//Calculate new influences
		for (int i = 0; i < amountOfNodes; i++)
		{
			if (!IsNodeValid(i)) continue;

			InfluenceNode* pNode = GetNode(i);
			float highestInfluence{};
			for (auto pCon : GetNodeConnections(pNode))
			{
				InfluenceNode* pSource = GetNode(pCon->GetTo());
				float newInfluence = pSource->GetInfluence() * expf(- pCon->GetCost() * m_Decay);

				if (abs(newInfluence) > abs(highestInfluence)) highestInfluence = newInfluence;
			}

			//float newInfluence = m_Momentum * pNode->GetInfluence() + (1 - m_Momentum) * highestInfluence;
			float newInfluence = Elite::Lerp(pNode->GetInfluence(), highestInfluence, 1-m_Momentum);
			m_InfluenceDoubleBuffer[i] = newInfluence;
		}

		//Copy new influence values into graph
		for (int i = 0; i < amountOfNodes; i++)
		{
			if (!IsNodeValid(i)) continue;

			InfluenceNode* pNode = GetNode(i);
			pNode->SetInfluence(m_InfluenceDoubleBuffer[i]);
		}
	}
