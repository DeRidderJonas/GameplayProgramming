...
public:
		...
		m_SAngle(1, nrOutputs)
	{
		...
		for (int i = 0; i < m_MemorySize; ++i)
		{
			...
			m_StateMatrixMemoryArr[i].SetAll(0.f);
			m_ActionMatrixMemoryArr[i].SetAll(0.f);
		}
	...
	void Reset() {
		...
		m_Location.x = Elite::randomFloat(-50, 50);
		m_Location.y = Elite::randomFloat(-50, 50);
		m_Angle = Elite::randomFloat(0, float(M_PI) * 2);
	}
	...
	void MutateMatrix(FMatrix& matrix, float mutationRate, float mutationAmplitude) {
		for (int c = 0; c < matrix.GetNrOfColumns(); ++c) {
			for (int r = 0; r < matrix.GetNrOfRows(); ++r) {
				if (Elite::randomFloat(0, 1) < mutationRate) {
					float update = Elite::randomFloat(-mutationAmplitude, mutationAmplitude);
