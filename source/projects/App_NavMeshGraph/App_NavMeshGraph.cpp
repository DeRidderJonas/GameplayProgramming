...
std::vector<Elite::Vector2> App_NavMeshGraph::FindPath(Elite::Vector2 startPos, Elite::Vector2 endPos)
{
	...
	//Copy the graph
	auto sharedPtrCopy{ m_pNavGraph->Clone() };
	auto copy{ static_cast<NavGraph*>(sharedPtrCopy.get()) };

	//Create extra node for the Start Node (Agent's position)
	int startIdx = copy->AddNode(new NavGraphNode(copy->GetNextFreeNodeIndex(), -1, startPos));
	for (auto lineIdx : startTriangle->metaData.IndexLines)
	{
		int nodeIdx{ copy->GetNodeIdxFromLineIdx(lineIdx) };
		if (nodeIdx == invalid_node_index) continue;
		
		float distance{ Elite::Distance(startPos, copy->GetNode(nodeIdx)->GetPosition()) };
		copy->AddConnection(new Elite::GraphConnection2D(startIdx, nodeIdx, distance));
	}

	//Create extra node for the End Node
	int endIdx = copy->AddNode(new NavGraphNode(copy->GetNextFreeNodeIndex(), -1, endPos));
	for (auto lineIdx : endTriangle->metaData.IndexLines)
	{
		int nodeIdx{ copy->GetNodeIdxFromLineIdx(lineIdx) };
		if (nodeIdx == invalid_node_index) continue;

		float distance{ Elite::Distance(endPos, copy->GetNode(nodeIdx)->GetPosition()) };
		copy->AddConnection(new Elite::GraphConnection2D(nodeIdx, endIdx, distance));
	}

	//Run A star on new graph
	auto pathFinder = AStar<NavGraphNode, GraphConnection2D>(copy, Elite::HeuristicFunctions::Chebyshev);
	auto nodes = pathFinder.FindPath(copy->GetNode(startIdx), copy->GetNode(endIdx));

	m_DebugNodePositions.clear();
	for (const Elite::NavGraphNode* node : nodes)
	{
		finalPath.push_back(node->GetPosition());
		m_DebugNodePositions.push_back(node->GetPosition());
	}
