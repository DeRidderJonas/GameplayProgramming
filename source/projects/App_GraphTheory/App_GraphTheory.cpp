...
void App_GraphTheory::Update(float deltaTime)
{
	...
	m_pGraph2D->SetNodesColor(m_pGraph2D->GetAllActiveNodes(), Elite::Color{ 0.f,0.f,1.f });

	auto connection = m_pGraph2D->GetAllConnections();
	...
	vector<Elite::GraphNode2D*> eulerPath = eulerFinder.FindPath(eulerianity);
	const int amountOfNodes{ int(eulerPath.size()) };
	float i{1.f};
	for (auto it = eulerPath.rbegin(); it != eulerPath.rend(); ++it)
	{
		float color{ eulerianity == Eulerianity::eulerian ? 1.f :  1.f * (i / amountOfNodes) };
		(*it)->SetColor(Elite::Color(color, color, color));
		i++;
	}
